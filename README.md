...
wrk go [<repo> <tree> <harness>] [--new] [-- <harness args>]
wrk pick [--json]
wrk ls [repo]
...
- `go` opens an existing tree that matches, or creates one when nothing matches. `--new` always creates a tree. `go` waits for `on_create`, then launches the harness in the tree. With no arguments it opens a picker.
