...
- `src/tui/` is the `go` picker. `state.rs` is a pure state machine with no terminal IO, `view.rs` renders it, and `mod.rs` owns the terminal.
...
- **`go` has one create → wait → exec path** (the private `go` function in `src/cmd/go.rs`). The picker returns a `GoTarget` and hands off to it. Don't add a second launch path.
